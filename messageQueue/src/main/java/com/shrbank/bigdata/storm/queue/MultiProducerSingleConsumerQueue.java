...
/**
 * Created by wushaojie on 2016/10/17.
 * 多生产者、单消费者非阻塞队列。具有容量限制
 */
...
public class MultiProducerSingleConsumerQueue<T> implements ISpoutMessageQueue<T> {
    ...
    public MultiProducerSingleConsumerQueue(final int capacity) {
        if (capacity < 0) throw new IllegalArgumentException("MultiProducerSingleConsumerQueue.capacity must be >= 0");
    ...
    static {
        try {
            enqOffset = Unsafe.instance.objectFieldOffset(MultiProducerSingleConsumerQueue.class.getDeclaredField("_enqDoNotCallMeDirectly"));
            deqOffset = Unsafe.instance.objectFieldOffset(MultiProducerSingleConsumerQueue.class.getDeclaredField("_deqDoNotCallMeDirectly"));
        } catch(Throwable t){
