...
object SyncSpoutClient {
  lazy private[SyncSpoutClient] val config = ConfigFactory.load(Utils.ActorConfigFile).withFallback(ConfigFactory.load(Utils.ClientConfigFile))

  lazy private[SyncSpoutClient] val client = ActorSystem.create(Utils.ClientSystemName,config)
  /**
...
class SyncSpoutClient{
  ...
  private def connect(host:String,port:Int):Unit = {
    val serverPath =  s"akka.tcp://${Utils.ServerSystemName}@$host:$port/user/$topologyName"
    ...
    try{
      ...
      val workerActorRef = Await.result(actorRefFuture,SyncSpoutClient.initTimeOut.duration).asInstanceOf[ActorIdentity].getActorRef.orElse(null)
      // 防止初始化多次
  ...
  /**
    ...
    */
  private def ask(msg:ClientMsg,timeout:Timeout):ServerMsg = {
    var returnMsg :ServerMsg = ServerMsg(null)
    try{
      nextServer() match {
        case Some(server) =>
          val future = Patterns.ask(server,msg,timeout)
          returnMsg = Await.result(future,timeout.duration).asInstanceOf[ServerMsg]
        case None=>
    ...
    log.debug(s"处理消息$msg，耗时${System.currentTimeMillis() - msg.sendTime} 毫秒")
    returnMsg
  }
  /**
    * 向server发送消息，并在指定时间内返回
    ...
    * @return 若在timeout之间内，SyncSpout有返回，则返回storm计算的结果，否则返回null
    */
  def ask(msg:ClientMsg,timeout:Long):ServerMsg = {
  ...
  /**
    ...
    * @return 若在timeout之间内，SyncSpout有返回，则返回storm计算的结果，否则返回null
    */
  def ask(msg:ClientMsg) :ServerMsg = {
  ...
  def destroy():Unit = {
   ...
   //    SyncSpoutClient.client.shutdown()
    SyncSpoutClient.client.terminate()
    this.zkConfig.close()
  }
}
