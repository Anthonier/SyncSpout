...
class SyncSpoutZkConfig {
  ...
  private def createRootPath() = {
    try{
      zkClient.create(SyncSpoutZkConfig.ZK_ROOT_PATH, "".getBytes, CreateMode.PERSISTENT)
    }catch {
      case ex:ZKNodeExistsException=>
    }
  }

  private def deleteRootPath() = zkClient.delete(SyncSpoutZkConfig.ZK_ROOT_PATH)
