...
class SimpleBolt extends BaseBasicBolt{
  ...
  override def execute(tuple: Tuple, basicOutputCollector: BasicOutputCollector): Unit = {
    val msg = "这是处理后的消息，可以添加自己的逻辑-" + tuple.getValueByField(SendBolt.OutputFieldName).
  ...
  override def declareOutputFields(outputFieldsDeclarer: OutputFieldsDeclarer): Unit = {
    ...
    outputFieldsDeclarer.declare(SyncBoltFields(SendBolt.OutputFieldName,"MessageLength"))
  }
}